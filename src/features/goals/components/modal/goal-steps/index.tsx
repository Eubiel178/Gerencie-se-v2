...
import { IGoalStep } from "@/features/goals/domain";

import styles from "./styles.module.css";

export interface GoalStepsDraft {
  existingSteps: IGoalStep[];
...
export function GoalSteps({ steps, onDraftChange }: GoalStepsProps) {
  ...
  function publish(
    nextExistingSteps: IGoalStep[],
    nextPendingSteps: { id: string; title: string }[],
  ) {
    onDraftChange({
  ...
  function updatePendingStep(id: string, title: string) {
    const nextPendingSteps = pendingSteps.map((step) =>
  ...
  function handleToggleStep(id: string, completed: boolean) {
    const nextSteps = visibleSteps.map((step) =>
          ...
          {pendingSteps.map((step) => (
            <li key={step.id} className={styles.step}>
              ...
              <button
                ...
                onClick={() => removePendingStep(step.id)}
                aria-label={`Remover novo passo ${step.title}`}
              >
            ...
            <li key={step.id} className={styles.step}>
              ...
              {editingStepId === step.id ? (
                <>
                  <button
                    ...
                    aria-label={`Salvar passo ${step.title}`}
                  >
              ...
              <div className={styles.orderActions} aria-label={`Reordenar ${step.title}`}>
                <button
                  ...
                  aria-label={`Mover ${step.title} para cima`}
                  disabled={index === 0}
                >
                ...
                <button
                  ...
                  className={styles.actionButton}
                  onClick={() => handleMoveStep(step.id, 1)}
                  aria-label={`Mover ${step.title} para baixo`}
