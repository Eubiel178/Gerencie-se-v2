...
import type { StepLike, StepsDraft } from "./types";

export type { StepLike, StepsDraft };

interface StepsEditorProps<T extends StepLike> {
  steps: T[];
  onDraftChange: (draft: StepsDraft<T>) => void;
  ariaLabel?: string;
}

export function StepsEditor<T extends StepLike>({
  steps,
  onDraftChange,
  ariaLabel,
}: StepsEditorProps<T>) {
  ...
  function publish(nextExistingSteps: T[], nextPendingSteps: { id: string; title: string }[]) {
    onDraftChange({
  ...
  return (
    <div className={styles.wrapper}>
      ...
      {visibleSteps.length > 0 && (
        <ul className={styles.steps} aria-label={ariaLabel}>
          {visibleSteps.map((step, index) => (
            <li key={step.id} className={styles.step}>
              ...
              <ConfirmIconButton icon="FaTrash" ariaLabel={`Remover passo "${step.title}"`} confirmText={`Remover o passo "${step.title}"?`} confirmLabel="Remover" className={styles.smallButton} onConfirm={() => handleRemoveStep(step.id)} />
            </li>
          ))}
        </ul>
      )}
