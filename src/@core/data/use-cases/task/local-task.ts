...
import type { TaskSyncStatus } from "@/@core/domain";
...
/**
 * Implementação local (Drizzle + SQLite) dos casos de uso de Task.
 *
 * Substitui a antiga `RemoteTask` (Axios + json-server). Só pode ser usada
 * no servidor — depende de `@libsql/client`, que não roda no browser. Por
